...
중략..
```
flag 값이 한번 나온다고 했으니 이 중에서 중복이 없는 줄을 구해야 한다

우선 sort 명령어를 이용해 data.txt 파일을 정렬해 보자
```
$ cat ./data.txt | sort
```
위와 같이  "|" 기호를 통해 앞에서 나온 데이터들을 sort 명령어로 전달할 수 있다
```
...
중략..
```
정렬은 됐지만 데이터의 양이 너무 많아서 유일한 데이터를 찾기 어려울 것 같다

중복된 값을 지우는 명령어인 uniq 와 중복 횟수를 카운팅하는 옵션 "-c" 를 통해 flag 를 찾아보자   
위와 마찬가지로 "|" 를 한번 더 사용하여 정렬된 데이터를 전달해 줄 수 있다
```
...
```
중복 횟수가 1회인 라인(flag)을 찾았다   


flag 값의 발견으로 Level 8 → Level 9 은 끝난다



Link: [overthewire.org](https://overthewire.org/wargames/bandit/bandit9.html)
